...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{../Images/topo_radius.png}
    \caption{AAA using Radius}
\end{figure}

The objective of this exercise was to configure centralized AAA (Authentication, Authorization, and Accounting) using the Radius protocol.

The topology consists of a Cisco 7200 router (R1), a Radius server implemented using the GNS3 AAA appliance, and a Toolbox appliance acting as a Telnet client. The router operates as a Radius client and forwards authentication requests to the AAA server.

Initially, IP connectivity between all devices was configured and verified using \texttt{ping}.
...
\begin{center}
\includegraphics[width=0.75\textwidth]{../Images/radius_login.png}
\end{center}

\paragraph{Authorization}

Radius authorization was configured for EXEC access after successful authentication.

Unlike TACACS+, Radius does not support per-command IOS authorization. As a result, it was not possible to restrict specific IOS commands for different users.
...
\begin{center}
\includegraphics[width=0.75\textwidth]{../Images/radius_accounting.png}
\end{center}

\subsubsection{Wireshark Analysis}

Wireshark was used to capture and analyze Radius traffic exchanged between the router and the AAA server.

Radius communication uses UDP ports 1812 and 1813. Authentication traffic uses port 1812, while accounting traffic uses port 1813.

\begin{center}
\includegraphics[width=0.9\textwidth]{../Images/radius_wireshark.png}
\end{center}

The captures show Radius Access-Request and Access-Accept messages during user authentication, as well as Accounting-Request and Accounting-Response messages during session establishment and termination.

Unlike TACACS+, Radius combines authentication and authorization functions and encrypts only the user password field instead of the full packet payload.
