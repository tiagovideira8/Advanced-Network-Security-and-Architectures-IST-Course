...
The attacker sent forged RIPv2 Response messages advertising a route to the legitimate Web server network through the attacker itself. In particular, the attacker announced the prefix \texttt{10.0.0.0/25}, which is more specific than the legitimate route. Due to the longest prefix match rule used by routers during route selection, the malicious route was preferred over the legitimate route, causing browser traffic to be redirected to the fake Web server. The forged RIPv2 packets were generated using a Python script implemented with Scapy, presented in \label{appendix:rip-script}.
...
The poisoning attack was also confirmed through the routing tables of the routers. Before the attack, router R2 learned the network \texttt{10.0.0.0/24} through the legitimate path via router R3. After the attack, the routing table contained the malicious route \texttt{10.0.0.0/25} pointing to the attacker (\texttt{172.16.0.2}), demonstrating that the forged RIP updates were successfully installed.
...
As a countermeasure, RIPv2 MD5 authentication was enabled on all router interfaces participating in RIP. With authentication enabled, routers only accept RIP updates containing the correct authentication information associated with the configured key-chain. Consequently, forged RIP messages generated by the attacker were rejected because they did not contain valid authentication data.

After enabling MD5 authentication,
routers only accepted RIP updates containing
valid authentication information associated
with the configured key-chain.

As a result, forged RIP packets generated
by the attacker were ignored,
preventing malicious routes from being installed
in the routing tables.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{../Images/RealServer.png}
    \caption{Browser successfully accessing the legitimate Web server after RIPv2 MD5 authentication was enabled}
    \label{fig:RealServer}
\end{figure}

Wireshark captures obtained after enabling authentication showed that legitimate RIP packets contained MD5 authentication information, while the forged packets sent by the attacker lacked authentication fields and were ignored by the routers.

During the experiments, previously learned malicious routes remained temporarily present in the routing tables even after authentication was enabled. The poisoned routes only disappeared after route expiration or after manually clearing the routing tables using IOS commands. This behavior is explained by RIP route timers, which preserve learned routes for a limited period before invalidation.

\subsubsection{Routing Table Analysis}

The routing table behavior observed during the experiments
confirmed how forged RIP advertisements influenced
route selection on the routers.

During the first attack attempt,
the attacker advertised the same network prefix
(\texttt{10.0.0.0/24}) with the same RIP metric
used by the legitimate route.

As a result, router R2 installed both routes simultaneously,
creating an equal-cost multipath situation.

\begin{configbox}
R2# show ip route

R    10.0.0.0 [120/1] via 192.168.23.3
                 [120/1] via 172.16.0.2
\end{configbox}

Since both routes had identical prefix lengths and metrics,
the malicious route did not consistently override
the legitimate path.

To increase the effectiveness of the attack,
the attacker then advertised a more specific prefix
(\texttt{10.0.0.0/25}) instead of the original
\texttt{10.0.0.0/24} network.

In this second scenario, router R2 preferred
the malicious route due to the longest prefix match rule,
even though the RIP metric was higher.

\begin{configbox}
R2# show ip route

R    10.0.0.0/25 [120/5] via 172.16.0.2
R    10.0.0.0/24 [120/1] via 192.168.23.3
\end{configbox}

This routing table behavior explains why browser traffic
was redirected towards the fake Web server during the attack.

The experiment therefore confirms that
route specificity has a stronger influence on forwarding decisions
than RIP metric values when longest prefix matching applies.
...
In the implemented topology, enabling RIP auto-summary did not significantly affect the success of the attack. The malicious route advertised by the attacker continued to be propagated and selected because the injected prefix remained more specific than the legitimate route. Therefore, the longest prefix match rule had a greater impact on route selection than the auto-summary feature in this particular scenario.
...
The attack is feasible in networks where RIP operates without authentication and where attackers can access the routing domain. Since RIP inherently trusts received routing updates, malicious devices can inject forged routes and manipulate traffic forwarding decisions. Although RIP is less common in modern enterprise networks, the experiment demonstrates how insecure routing protocols can enable traffic hijacking attacks when authentication mechanisms are not deployed.

The experiment also highlights the importance of route selection rules, particularly longest prefix matching, which allowed the malicious route to override the legitimate path. While modern routing protocols often provide stronger security mechanisms, similar routing manipulation attacks remain relevant in contemporary networking environments.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../Images/SendPass.png}
\caption{Detailed TACACS+ authentication packet showing password exchange metadata}
\label{fig:tacacs-sendpass}
\end{figure}

Detailed inspection of TACACS+ packets shows
the exchange of authentication messages between
the router and the TACACS+ server.

Wireshark identified TACACS+ packets associated
with password transmission and login validation,
including \texttt{Send Password} authentication exchanges.

The packets also contain protocol metadata such as:
...
Although the TACACS+ payload is encrypted,
Wireshark still exposes important control information
that allows the authentication process to be analyzed.
...
\section{RIP Poisoning Attack Script}
\label{appendix:rip-script}

This appendix contains the Python/Scapy script used
to generate forged RIPv2 Response packets during
the RIP poisoning attack.

The first implementation advertised the same prefix
and metric as the legitimate route, creating an
equal-cost multipath situation.

The second implementation advertised a more specific
prefix (\texttt{/25}), which successfully redirected
traffic due to the longest prefix match rule.

\subsection{First Attempt -- Equal-Cost Route Injection}
...
This first attempt created equal-cost routing entries
because the malicious route used the same prefix length
and RIP metric as the legitimate route.

\subsection{Second Attempt -- More Specific Prefix Injection}
...
In the second attempt, the attacker advertised
the more specific prefix \texttt{10.0.0.0/25}.

Although the RIP metric was higher,
routers preferred the malicious route because
of the longest prefix match rule,
successfully redirecting traffic towards
the fake Web server.
