...
\subsection{IKEv2 and IPSec Configuration}

Both routers were configured with:
\begin{itemize}
    \item an IKEv2 proposal using AES-CBC-256 encryption;
    \item SHA512 integrity and pseudo-random functions;
    \item Diffie-Hellman Group 16;
    \item an IPSec transform set using ESP-AES and ESP-SHA-HMAC;
    \item an IPSec profile applied to Tunnel0.
\end{itemize}

Figures~\ref{fig:r1_ikev2_config} and~\ref{fig:r2_ikev2_config} present the relevant configuration and verification commands executed on R1 and R2.
...
\begin{figure}[H]
    ...
    \label{fig:r2_ikev2_config}
\end{figure}

\subsection{Tunnel Establishment Verification}
...
\begin{itemize}
    \item successful establishment of inbound and outbound IPSec Security Associations;
    \item packet encapsulation and decapsulation through ESP;
    \item operational Tunnel0 interfaces;
    \item successful OSPF adjacency establishment through the protected tunnel.
\end{itemize}

Figure~\ref{fig:ipsec_sa} shows the active IPSec Security Associations and packet counters.
