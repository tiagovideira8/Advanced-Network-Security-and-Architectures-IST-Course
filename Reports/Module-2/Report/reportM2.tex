...
\begin{figure}[H]
    ...
    \caption{BGP Update messages exchanged between PE2 and PE1 advertising blue organization prefixes}
...
\begin{figure}[H]
    ...
    \caption{Contents of the BGP Update message, showing MP\_REACH\_NLRI with Route Distinguisher, VPN label, and EXTENDED\_COMMUNITIES with Route Target RT:1:1}
...
Figure~\ref{fig:topo_IPIP} shows the setup used in this exercise. Node \texttt{ub1} 
hosts namespace \texttt{blue} and node \texttt{ub2} hosts namespace \texttt{red}. 
Each namespace is connected to its host via a bridge interface and a veth pair. 
An IPIP tunnel (\texttt{tun0}) is established between the physical interfaces of 
the two hosts (\texttt{192.168.50.1} and \texttt{192.168.50.2}), and static routes 
on each host direct namespace traffic through the tunnel.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{../Images/IPIP topo.png}
    \caption{Topology}
    \label{fig:topo_IPIP}
\end{figure}
