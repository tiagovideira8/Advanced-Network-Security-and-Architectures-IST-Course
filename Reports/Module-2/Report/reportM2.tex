...
This section describes the configuration and analysis of an IPSec VPN using IKEv2 authentication with pre-shared keys. 
The objective is to establish a protected tunnel between routers R1 and R2 through the intermediate router RA and analyze the establishment and management of IKEv2 and IPSec Security Associations (SAs).

\subsection{IKEv2 and IPSec Configuration Verification}

The IKEv2 and IPSec configuration was verified on both R1 and R2 using IOS show commands.

The following elements were validated:

\begin{itemize}
    \item IKEv2 proposal defining encryption (AES-256), integrity (SHA512), PRF (SHA512), and Diffie-Hellman Group 16;
    \item IKEv2 policy referencing the defined proposal;
    \item IPSec transform set and profile associating IKEv2 with IPSec protection;
    \item IPSec Security Associations confirming correct encapsulation parameters.
\end{itemize}

Figures~\ref{fig:ikev2_prop_r1} and~\ref{fig:ikev2_prop_r2} show the IKEv2 proposal configuration on both routers.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../Images/show crypto ikev2 prop R1.png}
    \caption{IKEv2 and IPSec configuration verification on R1}
    \label{fig:proposal_r1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../Images/sh crypto ipsec sa R1.png}
    \caption{IKEv2 and IPSec configuration verification on R2}
    \label{fig:ipsec_sa_r2}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../Images/show crypto ikev2 prop R1.png}
        \caption{IKEv2 proposal verification on R1}
        \label{fig:ikev2_prop_r1}
    \end{minipage}
    \hfill
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../Images/show crypto ikev2 prop R2.png}
        \caption{IKEv2 proposal verification on R2}
        \label{fig:ikev2_prop_r2}
    \end{minipage}
...
A successful ping confirmed that:
\begin{itemize}
    \item the IPSec tunnel was operational;
    \item encrypted traffic was successfully forwarded between endpoints;
    \item routing across the protected tunnel was functional.
\end{itemize}

Figure~\ref{fig:ping_test} shows successful ICMP communication between the end hosts.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../Images}
    \caption{Successful ICMP connectivity between PC1 and PC2 through IPSec tunnel}
    \label{fig:ping_test}
\end{figure}
...
The IKE\_SA\_INIT exchange was responsible for:
\begin{itemize}
    ...
    \item creation of the initial IKE Security Association.
\end{itemize}

Figure~\ref{fig:ike_sa_init} presents the captured IKE\_SA\_INIT packets.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../Images/}
    \caption{IKE\_SA\_INIT exchange captured in Wireshark}
    \label{fig:ike_sa_init}
\end{figure}
...
\begin{itemize}
    ...
    \item SHA512 integrity protection;
    \item SHA512 pseudo-random function;
...
\subsection{Analysis of IKE\_AUTH Exchange}

After the IKE\_SA\_INIT exchange, the peers performed mutual authentication using the configured pre-shared key during the IKE\_AUTH exchange.

Figure~\ref{fig:ike_auth} presents the captured IKE\_AUTH packets.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{ike_auth.png}
    \caption{IKE\_AUTH exchange captured in Wireshark}
    \label{fig:ike_auth}
\end{figure}

Unlike the IKE\_SA\_INIT packets, the IKE\_AUTH payloads are encrypted and authenticated. 
Wireshark identifies these payloads as encrypted, demonstrating that cryptographic keys derived during the previous exchange are already being used to protect the communication.

The IKE\_AUTH exchange is responsible for:
\begin{itemize}
    \item peer authentication;
    \item identity exchange;
    \item creation of the first CHILD\_SA used for IPSec protection.
\end{itemize}
