...
Figure~\ref{fig:ike_sa_init} presents the captured IKEv2 exchange messages.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../Images/isakmp messages IKEv2.png}
    \caption{IKEv2 exchange messages captured in Wireshark}
...
Figure~\ref{fig:ike_auth_content} presents the contents of the encrypted IKE\_AUTH exchange.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../Images/Content of AUTH IKEv2.png}
    \caption{Contents of the IKE\_AUTH exchange}
    \label{fig:ike_auth_content}
\end{figure}

Unlike the IKE\_SA\_INIT exchange, the IKE\_AUTH payloads are encrypted and authenticated using keys derived during the initial exchange. This stage performs peer authentication using the configured pre-shared key and establishes the first CHILD\_SA used for IPSec protection.
