...
On the P--PE2 segment, the packet carries only a single label (value 19), as shown in 
Figure~\ref{fig:mpls_1_label}. This is a consequence of Penultimate Hop Popping 
(PHP): the P router, acting as the penultimate LSR before PE2, pops the outer 
transport label rather than swapping it, forwarding the packet to PE2 with only the 
inner VPN label exposed. PHP is enabled by default in Cisco IOS and is signaled 
through LDP by advertising the implicit-null label for directly connected prefixes, 
which instructs the upstream neighbor to pop rather than swap. This behavior is 
confirmed by PE2's LDP bindings, where the local binding for \texttt{2.2.2.2/32} 
shows \texttt{imp-null}, meaning PE2 told the P router to pop the transport label 
before delivery. As a result, PE2 receives packets with only the VPN label, avoiding 
a redundant label lookup.

PE2 uses this VPN label to determine the destination VRF and forward the packet to 
the correct CE. This is correlated with PE2's MPLS forwarding table shown in 
Figure~\ref{fig:mpls_pe2}, where label 19 maps to prefix 
\texttt{192.168.102.0/24[V]}, the \texttt{[V]} annotation indicating a VRF route. 
The outgoing label is \texttt{No Label}, confirming that PE2 disposes of the VPN 
label and delivers the packet natively into the blue VRF toward \texttt{10.10.2.100}. 
By the same logic as above, the transport label value 17 observed on the PE1--P link 
was assigned by the P router and advertised via LDP to PE1, not generated by PE1 
itself.

The VPN label is the mechanism that ensures the two organizations remain isolated even 
when using overlapping IP address spaces. Because the forwarding decision at PE2 is 
based on the MPLS label rather than the destination IP address, two packets carrying 
different VPN labels are delivered to different VRFs without ambiguity, regardless of 
whether their IP destinations overlap. The outer transport label changes at every LSR 
hop across the provider core through standard label swapping, while the inner VPN 
label remains unchanged from ingress PE to egress PE, where it is finally disposed.
